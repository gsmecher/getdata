 ...
 */
/* Test gd_alter_chunk_size: set chunk_size on empty field, then write+read */
#include "test.h"
...
int main(void)
{
  ...
  e2 = gd_putdata(D, "data", 0, 0, 0, 8, GD_UINT32, c);
  CHECKI(e2, 8);

  e3 = gd_getdata(D, "data", 0, 0, 0, 8, GD_UINT32, d);
  CHECKI(e3, 8);
