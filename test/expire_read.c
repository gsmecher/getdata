 ...
 */
/* Test gd_expire: after expiry, reads of deleted region return zeros */
#include "test.h"
...
int main(void)
{
  ...
  /* Read entire range including deleted region */
  e4 = gd_getdata(D, "data", 0, 0, 0, 8, GD_UINT32, d);
  CHECKI(e4, 8);

  gd_discard(D);

  /* Frames 0-3 were deleted: should read as zero */
  for (i = 0; i < 4; ++i)
    CHECKUi(i, d[i], 0);

  /* Frames 4-7 should be preserved */
  for (i = 4; i < 8; ++i)
    CHECKUi(i, d[i], c[i]);
