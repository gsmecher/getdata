...
int main(void)
{
  ...
  const char *data_zst = "dirfile/data.zst";
...
#ifdef USE_ZSTD
  D = gd_open(filedir, GD_RDWR | GD_ZSTD_ENCODED | GD_VERBOSE);
#else
  D = gd_open(filedir, GD_RDWR | GD_ZSTD_ENCODED);
#endif
